...
function extractMarksFromPerformanceText(perfText) {
  const text = cleanText(perfText || "");
  if (!text) return [];

  const rows = [];
  const directMatches = [...text.matchAll(/([A-Za-z][A-Za-z0-9-]*)\s*\/\s*(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)/g)];
  for (const match of directMatches) {
    const componentName = cleanText(match[1] || "");
  ...
  const componentDefs = [...text.matchAll(/([A-Za-z][A-Za-z0-9-]*)\s*\/\s*(\d+(?:\.\d+)?)/g)].map((match) => ({
    componentName: cleanText(match[1] || ""),
    maxScore: Number(match[2])
  })).filter((item) => item.componentName && Number.isFinite(item.maxScore) && item.maxScore > 0);

  if (componentDefs.length === 0) {
    return [];
  }

  const remainder = cleanText(text.replace(/([A-Za-z][A-Za-z0-9-]*)\s*\/\s*(\d+(?:\.\d+)?)/g, " "));
  const scoreTokens = [...remainder.matchAll(/\b(\d+(?:\.\d+)?)\b/g)].map((match) => Number(match[1])).filter((value) => Number.isFinite(value));
  if (scoreTokens.length < componentDefs.length) {
    return [];
  }

  for (let index = 0; index < componentDefs.length; index += 1) {
    const component = componentDefs[index];
    const score = Number(scoreTokens[index]);
    if (!Number.isFinite(score) || score < 0 || score > component.maxScore) continue;
    rows.push({
    ...
    for (let headerAt = 0; headerAt < tableRows.length; headerAt += 1) {
      ...
      for (const cells of tableRows.slice(headerAt + 1)) {
        ...
        if (idxTestPerformance >= 0) {
          ...
          const extractedRows = extractMarksFromPerformanceText(perf);
          for (const row of extractedRows) {
            pushComponent(row.componentName, row.score, row.maxScore);
          }
        }
...
function parseMarksRows(html) {
  ...
  for (const tableRows of tables) {
    ...
    for (const cells of tableRows.slice(headerAt + 1)) {
      ...
      const matches = extractMarksFromPerformanceText(perf);
      if (matches.length > 0) {
        lastComponentName = cleanText(matches[0].componentName || "").slice(0, 255);
      }
      for (const m of matches) {
        const componentName = String(m.componentName || "").trim();
        const maxScore = Number(m.maxScore);
...
function parseMarksRowsFromText(text) {
  ...
  for (const line of lines) {
    ...
    const subjectWithInlineMarks = line.match(/^([0-9]{2}[A-Z]{2,}\d+[A-Z]?)\s+(.+)$/i);
    if (subjectWithInlineMarks) {
      const code = normalizeCourseCode(subjectWithInlineMarks[1]);
      const remainder = cleanText(subjectWithInlineMarks[2] || "");
      const perfStart = remainder.search(/([A-Za-z][A-Za-z0-9-]*)\s*\/\s*(\d+(?:\.\d+)?)|\b\d+(?:\.\d+)?\s*\/\s*\d+(?:\.\d+)?\b/i);
      const subjectSegment = perfStart > 0 ? remainder.slice(0, perfStart).trim() : remainder;
      const normalizedSubjectSegment = subjectSegment.replace(/\b(theory|practical|lab)\b/gi, "").replace(/\s+/g, " ").trim();
      currentSubject = (courseTitleByCode.get(code) || normalizedSubjectSegment || code || remainder).slice(0, 255);
      pendingComponent = null;
      pendingMaxScore = null;
      pendingComponents = [];
      if (perfStart > 0) {
        workingLine = remainder.slice(perfStart).trim();
      } else if (perfStart === 0) {
        workingLine = remainder;
      } else {
        continue;
      }
    } else if (
    ...
    const extractedInlineRows = extractMarksFromPerformanceText(workingLine);
    if (extractedInlineRows.length > 0) {
      for (const row of extractedInlineRows) {
        parsed.push({
          id: createId("acad"),
          subjectName: (currentSubject || "Unknown Subject").slice(0, 255),
          componentName: cleanText(row.componentName).slice(0, 255),
          score: row.score,
          maxScore: row.maxScore,
          percentage: Number(((row.score / row.maxScore) * 100).toFixed(2))
        });
      }
      pendingComponent = null;
      pendingMaxScore = null;
    ...
    // line A: "FT-I/5.00"
    // line B: "2.50"
    const componentOnlyMatches = [...workingLine.matchAll(/([A-Za-z0-9-]+)\s*\/\s*(\d+(?:\.\d+)?)/g)];
    if (componentOnlyMatches.length > 0 && !/\s+\d+(?:\.\d+)?/.test(workingLine.replace(/([A-Za-z0-9-]+)\s*\/\s*(\d+(?:\.\d+)?)/g, "").trim())) {
      for (const match of componentOnlyMatches) {
        const componentName = cleanText(match[1]).slice(0, 255);
        const maxScore = Number(match[2]);
        if (!componentName || !Number.isFinite(maxScore) || maxScore <= 0) continue;
        pendingComponents.push({ componentName, maxScore });
      }
      if (pendingComponents.length > 0) {
        pendingComponent = pendingComponents[0].componentName;
        pendingMaxScore = pendingComponents[0].maxScore;
      }
      continue;
    }

    const scoreOnly = workingLine.match(/^(\d+(?:\.\d+)?)$/);
    if (scoreOnly && ((Number.isFinite(pendingMaxScore) && pendingMaxScore > 0) || pendingComponents.length > 0)) {
      const activePending = pendingComponents.length > 0
        ? pendingComponents.shift()
        : { componentName: pendingComponent, maxScore: pendingMaxScore };
      const activeName = cleanText(activePending?.componentName || pendingComponent || "overall").slice(0, 255);
      const activeMax = Number(activePending?.maxScore ?? pendingMaxScore);
      const score = Number(scoreOnly[1]);
      if (Number.isFinite(score) && Number.isFinite(activeMax) && activeMax > 0 && score >= 0 && score <= activeMax) {
        parsed.push({
          id: createId("acad"),
          subjectName: (currentSubject || "Unknown Subject").slice(0, 255),
          componentName: activeName,
          score,
          maxScore: activeMax,
          percentage: Number(((score / activeMax) * 100).toFixed(2))
        });
      }
      pendingComponent = pendingComponents[0]?.componentName || null;
      pendingMaxScore = pendingComponents[0]?.maxScore || null;
      continue;
    }

    // Example: "FT-I 5.0/5.0" or "5.0/5.0" with previous component line.
    const ratioInline = workingLine.match(/^([A-Za-z][A-Za-z0-9 -]{1,40})?\s*[:\-]?\s*(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)(?:\s*%?)$/i);
    if (ratioInline) {
      const queuedPending = !ratioInline[1] && pendingComponents.length > 0 ? pendingComponents.shift() : null;
      const inlineComponent = cleanText(ratioInline[1] || queuedPending?.componentName || pendingComponent || "overall").slice(0, 255);
      const score = Number(ratioInline[2]);
      const maxScore = Number(ratioInline[3] || queuedPending?.maxScore);
    ...
    if (/^(ft|quiz|internal|assignment|assign|lab|practical|mid|end|cie|cat)[\w\s\-()]*$/i.test(workingLine)) {
      pendingComponent = cleanText(workingLine).slice(0, 255);
      pendingMaxScore = null;
      pendingComponents = pendingComponent ? [{ componentName: pendingComponent, maxScore: null }] : [];
    }
  }
...
async function scrapeAcademiaDataWithHttp({ collegeEmail, collegePassword, scrapeMode = "full" }) {
  ...
  const marksFromHtml = dedupeMarksRows([
    ...(marksPage ? parseMarksRows(marksPage.text) : []),
    ...(attendancePage ? parseMarksRows(attendancePage.text) : [])
  ]);
  ...
  const marks = needsMarks
    ? dedupeMarksRows([
        ...(marksFromHtml || []),
        ...parseMarksRowsFromText(marksText),
        ...parseMarksRowsFromText(attendanceText)
      ])
    : [];
...
async function captureAcademiaStorageState({ collegeEmail, collegePassword, outputPath, headless = false }) {
  if (!playwright) {
    throw new Error("Playwright is not installed. Run `cd gak-backend && npm i playwright` to enable SRM Academia scraping.");
  }
  ...
  const browser = await playwright.chromium.launch({ headless: Boolean(headless) });
